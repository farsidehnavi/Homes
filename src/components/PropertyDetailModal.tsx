        ...
        <div className="p-5 border-b border-slate-200 flex items-start justify-between gap-4 bg-slate-50/80 sticky top-0 z-10">
          <div>
            ...
            <h3 className="text-base sm:text-lg font-bold text-slate-900 leading-snug">
              اطلاعات کامل ملک در {item.address.split('،')[1] || 'نجف‌آباد'}
            </h3>
          </div>
        ...
        <div className="p-5 sm:p-6 space-y-6 flex-1">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-start justify-between gap-3">
            ...
            <button
              ...
              className="shrink-0 flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg bg-white border border-slate-300 text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer"
            >
              {copied ? (
                <>
          ...
          <div className="bg-gradient-to-br from-indigo-50 to-blue-50/50 border border-indigo-100 rounded-xl p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:border-r sm:border-indigo-100 sm:pr-4">
                ...
                <div className="text-xs text-slate-500 mt-1">
                  مساحت کل: {fmt(item.area)} متر مربع
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
              ...
              <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                <div className="flex items-center gap-1.5 text-slate-500 mb-1">
                  ...
                  <span>طبقه / تعداد طبقات</span>
                </div>
                <div className="font-bold text-slate-900 text-sm">
                  {item.floor === 0 ? 'همکف' : `طبقه ${fmt(item.floor)}`} (از {fmt(item.totalFloors)} طبقه)
                </div>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                ...
                <div className="font-bold text-slate-900 text-sm">
                  {item.age === 0 ? 'نوساز (کلید نخورده)' : `${fmt(item.age)} سال ساخت`}
                </div>
              </div>
